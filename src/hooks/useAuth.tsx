...
export default function useAuth(props: any) {
  const { setLoading, setError, toggle, setToggle } = useVariablesContext();

  const { email, password, setKeyUser, setCorrectData } = useUserContext();

  const { setTitle, setTitleBoton, setMensaje } = useAuthContext();
